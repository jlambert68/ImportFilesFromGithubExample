...
func main() {
	...
	githubFilesFiltered = filterFileListFromGitHub(githubFiles, &fileRegExFilterMap)
	...
	dropDownButton := widget.NewButton("Select Options", func() {
		popUp := widget.NewPopUp(checkboxList, myWindow.Canvas())
		popUp.Show()
	...
	*/
	...
	myTopLayout := container.NewVBox(button, dropDownButton, pathRow) //, pathLabel2)
...
func filterFileListFromGitHub(githubFiles []GitHubFile, fileRegExFilterMapPtr *map[string]string) (githubFilesFiltered []GitHubFile) {
	var fileRegExFilterMap map[string]string
	fileRegExFilterMap = *fileRegExFilterMapPtr

	var fullRegExFilter string
	var tempRegex string

	for fileilter, _ := range fileRegExFilterMap {
		if fileilter == "*.*" {
			tempRegex = ".*"
		} else {

			tempRegex = strings.ReplaceAll(fileilter, "*", "\\")
		}
		tempRegex = tempRegex + "$"
	...
	for _, githubFile := range githubFiles {
		if combinedRegex.MatchString(githubFile.Name) == true || githubFile.Type == "dir" {
			githubFilesFiltered = append(githubFilesFiltered, githubFile)
		}
	}

	return githubFilesFiltered
}
